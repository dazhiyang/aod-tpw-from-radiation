"""
2.create_holdout (QIQ): Extracts a strict 30% global holdout test set before LHS sampling.

Reads ``Data/qiq_1min_merra_qc.txt`` from ``Code/old/1.arrange.py``. For PAL/TAT (or any
``Data/<STATION>_<year>_all.txt``), use ``Code/2.create_holdout.py`` and edit its **CONFIG** block.
"""
