"""
3.latin_hypercube (QIQ / legacy): LHS sample from ``Data/trainpool.txt``.

For **PAL/TAT** station pools from ``2.create_holdout.py``, use ``Code/3.latin_hypercube.py`` and edit
its **CONFIG** (or set ``LHS_INPUT`` / ``LHS_TRAIN`` / ``LHS_N`` here for one-off runs).
"""
